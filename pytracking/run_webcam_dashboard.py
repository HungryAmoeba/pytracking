...
def run_webcam(tracker_name, tracker_param, debug=None, visdom_info=None, bagging = False):
    ...
    tracker.run_webcam_dashboard(debug, visdom_info, bagging)
...
def main():
    ...
    parser.add_argument('--bagging', type = bool, default = False, help='True to save final summary patches')

    args = parser.parse_args()

    visdom_info = {'use_visdom': args.use_visdom, 'server': args.visdom_server, 'port': args.visdom_port}
    run_webcam(args.tracker_name, args.tracker_param, args.debug, visdom_info, args.bagging)
