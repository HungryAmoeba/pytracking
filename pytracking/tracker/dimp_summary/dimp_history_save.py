...
class DiMP(BaseTracker):
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        for ind in list(range(self.summary_patches.size()[0])):
            self.summary_patches[ind] = (torch.moveaxis((img-img.min())/(img.max()-img.min()), 0, 2))
    ...
    def init_memory(self, train_x: TensorList):
        ...
        self.num_stored_samples = self.online_start_ind.copy()
        ...
        for sw, init_sw, num in zip(self.sample_weights, init_sample_weights, self.num_init_samples):
            sw[:num] = init_sw
        '''
        #for sw, summary_w, num, summary_num in zip(self.sample_weights, summary_sample_weights, self.num_init_samples, self.summary_size):
        #    sw[num:(num+summary_num)] = summary_w
        # Initialize memory
    ...
    def update_memory(self, sample_x: TensorList, target_box, im_patch, learning_rate = None):
        ...
        if summary_replace_ind != -1:
            self.training_samples[0][summary_replace_ind] = sample_x
    ...
    def update_sample_weights(self, sample_weights, previous_replace_ind, num_stored_samples, num_init_samples, sample_x = None, learning_rate = None):
        ...
        import pdb; pdb.set_trace()
        summary_replace_ind, _ = kc.online_summary_update_index(extremum_summary_set, observation, self.params.summary_size)
