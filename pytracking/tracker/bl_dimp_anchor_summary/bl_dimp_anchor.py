...
class BL_DiMP(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.ref_im_patches = torch.zeros(self.params.get('sample_memory_size'), self.params.image_sample_size, self.params.image_sample_size, 3)
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if self.params.get("use_summary", False):
            obj_id = 0
            self.summary_updated[obj_id] = False
            self.query_requested[obj_id] = False
            self.request_training_step[obj_id] = False
            self.summary_score[obj_id] = torch.tensor([-1], device=self.params.device)
        ...
        if self.params.get('use_limited_bandwidth', False):
            obj_id = 0
            self.frames_since_last_query[obj_id] += 1

        out = {'target_bbox': output_state}

        if self.params.get("log_summary", False):
            obj_id = 0
            out['summary_threshold'] = self.extremum_summary_threshold[obj_id].item()
            out['query_requested'] = self.query_requested[obj_id]
            out['summary_updated'] = self.summary_updated[obj_id]
            out['summary_size'] = self.num_init_samples[obj_id] #todo: fix this
            out['summary_score'] = self.summary_score[obj_id].item()
        return out
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        for i, img in enumerate(im_patches):
            ...
            self.ref_im_patches[i,...] = im_patch
    ...
    def init_memory(self, train_x: TensorList):
        ...
        self.num_objs = len(self.num_stored_samples)

    def init_xs_memory(self):
        obj_id = 0

        # Summary related
        self.extremum_summary_threshold = TensorList([torch.tensor([self.params.get('default_summary_threshold', 0.0)], device=self.params.device) for x in self.num_stored_samples])
        self.summary_score = TensorList([torch.tensor([-1], device=self.params.device) for x in self.num_stored_samples])
        self.summary_im_patches = TensorList([torch.zeros(self.num_init_samples[obj_id], self.params.image_sample_size,
                                           self.params.image_sample_size, 3, device=self.params.device) for x in self.num_stored_samples])

        self.summary_updated = [False] * self.num_objs
        self.request_training_step = [False] * self.num_objs

        self.summary_im_patches[obj_id] = self.ref_im_patches[:self.num_init_samples[obj_id], ...]

        # Initialize threshold, todo: make this a parameter
        self.extremum_summary_threshold[obj_id], _ = kc.get_mean_summary_score(
            self.training_samples[obj_id][:self.num_init_samples[obj_id], ...],
            dist_func=self.params.get("dist_func", "cosine_dist"))

        # Query related
        self.query_im_patch = [None] * self.num_objs
        self.query_sample = [None] * self.num_objs
        self.query_target_box = [None] * self.num_objs
        self.query_summary_score = [None] * self.num_objs
        self.frames_since_last_query = [0] * self.num_objs
        self.query_requested = [False] * self.num_objs
        self.query_replace_ind = [-1] * self.num_objs

        self.query_pred_bb = [None] * self.num_objs
        self.query_oracle_bb = [None] * self.num_objs
        self.query_frame_num = [-1] * self.num_objs

    def update_xs_memory(self, sample_x: TensorList, target_box, im_patch, learning_rate):
        # todo: handle multiple objects correctly

        obj_id = 0
        sample = sample_x[obj_id]

        # todo: change this to allow flexible
        summary_samples = self.training_samples[obj_id][:self.num_init_samples[obj_id]]
        summary_score, _, _ = kc.get_summary_score(summary_samples,
                                                   sample,
                                                   dist_func=self.params.get("dist_func", "cosine_dist"))
        ...
        query_policy = self.params.get("query_policy", "summary_threshold")

        if query_policy == "fixed_rate":
            replace_prev_query = self.query_sample[obj_id] is None or summary_score > self.query_summary_score[obj_id]
        elif query_policy == "summary_threshold":
            replace_prev_query = summary_score > self.extremum_summary_threshold[obj_id] and \
                (self.query_sample[obj_id] is None or summary_score > self.query_summary_score[obj_id])

        # Update the query sample if needed
        if replace_prev_query:
            self.query_sample[obj_id] = sample
            self.query_target_box[obj_id] = target_box
            self.query_summary_score[obj_id] = summary_score
            self.query_im_patch[obj_id] = im_patch

            # oracle parameters
            self.query_frame_num[obj_id] = self.frame_num
            self.query_oracle_bb[obj_id] = self.ground_truth_bbox
        ...
        if replace_ind > -1:
            if self.params.get("use_limited_bandwidth", False) and self.query_sample[obj_id] is not None:
                self.query_requested[obj_id] = True

                # Compute oracle errors
                pred_bbox = self.query_pred_bb[obj_id]
                ...
                gt = self.query_oracle_bb[obj_id]
                iou = calc_iou_overlap(torch.tensor(gt).unsqueeze(0), pred_bbox.unsqueeze(0))

                replace_ind = self.query_replace_ind[obj_id]
                sample = self.query_sample[obj_id]
                target_box = self.query_target_box[obj_id]
                im_patch = self.query_im_patch[obj_id]
                weight = self.sample_weights[obj_id][0]  # todo: should be from params

                # Clear the query
                self.query_summary_score[obj_id] = -1
                self.query_sample[obj_id] = None
                self.query_im_patch[obj_id] = None
                self.query_target_box[obj_id] = None

                self.query_frame_num[obj_id] = -1
                self.query_oracle_bb[obj_id] = None
                self.query_pred_bb[obj_id] = None

                self.frames_since_last_query[obj_id] = 0
            ...
            # Update the summary
            self.update_summary(replace_ind, sample, target_box, im_patch, weight)
            self.summary_updated[obj_id] = True
    ...
    def update_summary(self, replace_ind, sample, target_box, im_patch, weight):
        obj_id = 0
        mem_offset = 0 # used if replacing a different portion of the samples

        self.training_samples[obj_id][mem_offset + replace_ind, ...] = sample
        self.target_boxes[mem_offset + replace_ind, ...] = target_box
        img = im_patch.reshape(im_patch.size()[1:])
        img = torch.moveaxis((img - img.min()) / (img.max() - img.min()), 0, 2)
        self.summary_im_patches[obj_id][mem_offset + replace_ind, ...] = img
        self.ref_im_patches[mem_offset + replace_ind, ...] = img
        self.sample_weights[obj_id][mem_offset + replace_ind] = weight #self.sample_weights[0][0]  # todo: make this a parameter
    ...
    def update_classifier(self, train_x, target_box, im_patch, learning_rate=None, scores=None):
        ...
        if self.params.get("use_summary", False):
            self.update_xs_memory(TensorList([train_x]), target_box, im_patch, learning_rate)
    ...
    def return_summary_patches(self):
        return self.summary_im_patches[0]
