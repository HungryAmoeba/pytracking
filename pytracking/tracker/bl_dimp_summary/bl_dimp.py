...
class BL_DiMP(BaseTracker):
        ...
        if self.params.get("use_global_trainer", False):
            ...
            if self.params.get("use_global_extremum", True):
                ...
                for i, im_info in enumerate(zip(info["all_frames"], info["all_bboxes"])):
                    ...
                    replace_ind, _, _ = kc.online_summary_update_index_extremum(summary_samples[:curr_summary_size,...],
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if self.params.get("log_summary", False):
            ...
            out['summary_updated'] = self.summary_updated
            out['summary_size'] = self.num_stored_samples[0] - self.num_init_samples[0]
            out['summary_score'] = self.summary_score.item()
        return out
        ...
        if self.summary_stored_samples < self.summary_size[0]:
            ...
            if self.del_summary_num != -1:
                ...
                if self.use_saved_threshold:
                    summary_replace_ind, _, new_threshold = kc.online_summary_update_index(extremum_summary_set, observation, self.params.summary_size, threshold = self.saved_threshold)
                else:
                    summary_replace_ind, _, new_threshold = kc.online_summary_update_index(extremum_summary_set, observation, self.params.summary_size)
