...
import pytracking.libs.gamma_threshold as gt
...
class DiMP(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        # Get starting gamma and other parameters, default 2
        self.gamma = self.params.get('gamma', 1.1)

        # Initialize classifier
        self.init_classifier(init_backbone_feat)


        self.threshold = gt.threshold_cost(self.training_samples[0])
        self.observation_score = 0
        self.previous_image = self.training_samples[0][0]
        self.previous_img_distance = 0
    ...
    def track(self, image, manual_replace = -1, info: dict = None) -> dict:
        ...
        out = {'target_bbox': output_state, 'frame_num': self.frame_num, 'threshold': self.threshold,
                        'stored_samples': self.num_stored_samples, 'observation_score': self.observation_score,
                        'previous_img_distance': self.previous_img_distance}
        return out
    ...
    def update_memory(self, sample_x: TensorList, target_box, learning_rate = None):
        ...
        replace_ind = self.update_sample_weights(self.sample_weights, self.previous_replace_ind, self.num_stored_samples, self.num_init_samples, sample_x[0], learning_rate)
        #import pdb; pdb.set_trace()
        #print(f"{self.num_stored_samples[0]}", end = ',')
        #print(f"{self.threshold}", end = ',')

        #print(f"{self.observation_score}", end = ',')
    ...
    def update_sample_weights(self, sample_weights, previous_replace_ind, num_stored_samples, num_init_samples, sample_x, learning_rate = None):
        ...
        for sw, prev_ind, num_samp, num_init in zip(sample_weights, previous_replace_ind, num_stored_samples, num_init_samples):
            ...
            if num_samp == 0 or lr == 1:
                ...
                if num_samp < sw.shape[0]:
                    r_ind = -1
                    #import pdb; pdb.set_trace()
                    summary_output = gt.update_summary(self.training_samples[0][0:num_samp], sample_x, self.threshold)
                    self.observation_score= summary_output[1]
                    if summary_output[0]:
                        r_ind = num_samp
                        #print(f"Threshold before is {self.threshold}")
                        self.threshold = self.threshold * self.gamma
                        #print(f"made update, num samp is now {num_samp}, threshold is now {self.threshold}")
    ...
    def update_classifier(self, train_x, target_box, learning_rate=None, scores=None):
        ...
        self.previous_img_distance = torch.norm(train_x - self.previous_image)
        self.previous_image = train_x
        made_update = 0
        if hard_negative_flag or self.frame_num % self.params.get('train_sample_interval', 1) == 0:
            made_update = self.update_memory(TensorList([train_x]), target_box, learning_rate)

        #UPDATE FOR GAMMA DIMP
