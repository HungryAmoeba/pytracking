...
class Tracker:
    ...
    def run_sequence(self, seq, visualization=None, debug=None, ssup_mode=0, visdom_info=None, multiobj_mode=None):
        ...
        output = self._track_sequence(tracker, seq, init_info,ssup_mode=ssup_mode)
        return output

    def _track_sequence(self, tracker, seq, init_info,ssup_mode=0):
        # Define outputs
        ...
        out, training_samp = tracker.initialize(image, init_info)
        if out is None:
            out = {}
        #should eventually make this more general
        plotting = False
        '''
        print(f"size of training_samp is {training_samp.size()}")
        training = tracker.training_samples
        training_samp = training_samp.reshape(15, 512*18*18)
        n_neighbors = 10
        n_components = 2
        isomap = manifold.Isomap(n_neighbors = n_neighbors, n_components = n_components)
        Y = isomap.fit_transform(training_samp.cpu())
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            print(f"frame number is {frame_num}")
            while True:
            ...
            #manual replace is used if option 3 is selected
            manual_replace = -1
            if frame_num == 1:
                training_patches = tracker.return_training_patches()
                vs.draw_to_plot(fig, ax, num_row, num_col, training_patches)
            if ssup_mode:
                continuation = input("Otps: 1: original aug, 2: Summary, 3: manual replace  ")
                if continuation == "1" or continuation == "12":

                    im_patches = tracker.return_original_patches()
                    vs.visualize_patches(im_patches, "Original augmentations")

                if continuation =="2" or continuation == "12":
                    training_patches = tracker.return_training_patches()
                    vs.visualize_patches(training_patches, "Summary training set", labels = False)
                if continuation == "3":
                    training_patches = tracker.return_training_patches()
                    vs.visualize_patches(training_patches, "Summary training set", labels = True)
                    manual_replace = int(input("Enter index of image to replace: "))

            start_time = time.time()

            '''if frame_num == 36:
                training_samp = torch.tensor(tracker.training_samples[0])
                vec_length = torch.prod(torch.tensor(training_samp.size()[1:]))
                training_samp = training_samp.reshape(50, vec_length)
                n_neighbors = 10
                n_components = 2
                isomap = manifold.Isomap(n_neighbors = n_neighbors, n_components = n_components)
                Y = isomap.fit_transform(training_samp.cpu())
            ...
            out, new_image, r_ind = tracker.track(image, manual_replace, info)
            #print(im_patch.size())
            new_image = new_image.reshape(1,torch.prod(torch.tensor(new_image.size())))
            if frame_num != 1 and r_ind[0] != -1:
                training_patches = tracker.return_training_patches()
                #import pdb; pdb.set_trace()
                vs.update_at_index(fig,ax, num_row, num_col, training_patches, r_ind[0]-15)
            '''if plotting:
                embedding = torch.tensor(isomap.transform(new_image.cpu()))[0]
                ax.scatter([embedding[0]], [embedding[1]], c = 'g')
                ax.plot([embedding[0]], [embedding[1]], c = 'b')
                plt.draw()
                plt.pause(.01)'''
    ...
    def run_webcam(self, debug=None, visdom_info=None):
        ...
        while True:
            ...
            if len(sequence_object_ids) > 0:
                ...
                manual_replace = -1
                #out = tracker.track(frame, manual_replace, info = info)
