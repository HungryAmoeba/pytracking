...
class MultiObjectWrapper:
    ...
    def return_summary_patches(self):
        for obj_id in self.initialized_ids:
            return self.trackers[obj_id].return_summary_patches()

    def delete_summary_num(self, index):
        for obj_id in self.initialized_ids:
            self.trackers[obj_id].delete_summary_num(index)

    def update_bbox(self, index, bbox):
        for obj_id in self.initialized_ids:
            self.trackers[obj_id].update_bbox(index, bbox)

    def return_summary_bbox(self):
        for obj_id in self.initialized_ids:
            return self.trackers[obj_id].return_summary_bbox().cpu()

    def set_lock_true(self):
        for obj_id in self.initialized_ids:
            return self.trackers[obj_id].set_lock_true()

    def set_lock_false(self):
        for obj_id in self.initialized_ids:
            return self.trackers[obj_id].set_lock_false()
