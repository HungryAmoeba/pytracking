...
def distance_cost(summary_set, observation, distance_function = torch.norm):
...
def return_furthest_point(summary_set, observation_set, distance_function = torch.norm):
...
def score_observation(summary_set, observation, distance_function = torch.norm):
...
def extremumsummary(observation_set, num_observations):
  ...
  while summary.size()[0] < num_observations:
    ...
    _, best_index = return_furthest_point(summary, observation_set)
...
def online_summary_update_index(summary_set, observation, k, threshold = None):
    ...
    if score_observation(summary_set, observation) > threshold:
      ...
      dummy_summary_set = [x for x in torch.cat((summary_set, observation), dim = 0)]
        ...
        return -1, threshold
        #find something to return that won't break things.
    ...
    if len(dummy_summary_set) > k:
      ...
      indices_in_new_summary = extremumsummary_indexes(dummy_summary_set, k)
      indices_before = set(range(k))
      index_to_remove = indices_before - indices_in_new_summary
      '''print(f"indices before is {indices_before} \n ________________")
      print(f"indices in new summary is {indices_in_new_summary}")
...
def extremumsummary_indexes(observation_set, num_observations):
  ...
  while summary.size()[0] < num_observations:
    #choose samples to add to summary by finding samples farthest away from samples in current summary
    _, best_index, original_index = return_furthest_point_index(summary, observation_set) ## here we would like to ensure that we return the original index as well as current index
...
def return_furthest_point_index(summary_set, observation_set: IndexedTensor, distance_function = torch.norm):
  '''This function works in more or less an offline fashion. It takes as input an entire summary
  set and the entire set of observations and returns the index of the observation set that is
...
def calc_pairwise_distances(input_tensor, distance_function = torch.norm):
